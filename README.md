...
Рабочий MVP Telegram Mini App для изучения арабского языка через короткие синхронизированные аудиодиалоги и интервальное повторение FSRS.

Один диалог — одна карточка FSRS. Пользователь слушает урок целиком, оценивает понимание всего диалога, а сервер рассчитывает и сохраняет следующее повторение в Cloudflare D1.
...
- реальные интервалы, рассчитанные `ts-fsrs`, без хардкода;
...
Блок «Знакомство» из четырёх уроков находится в `content/lessons.ts`. Готовые записи лежат в `public/audio` под именами `block-01-lesson-01.mp3` — `block-01-lesson-04.mp3`.
...
lib/
  ...
  fsrs.ts                 адаптер ts-fsrs и сериализация карточек
...
tests/                    FSRS, auth, content, idempotency, cron
...
- каждый показ вариантов FSRS получает одноразовый `attempt_id` и снимок четырёх рассчитанных исходов;
...
- три записи немного короче целевого диапазона 20–40 секунд, но полностью поддерживаются плеером;
- один учебный блок и один уровень;
...
- параметры FSRS общие для всех пользователей и ещё не оптимизируются по накопленной истории.
